...
async CreateTokenAccountTransaction(owner:PublicKey,token:BridgeToken):Promise<Transaction|undefined>{
    return new Promise(async(resolve,reject) =>{
        ...
        try{
                ...
                if(!transaction) throw new Error(SolanaError.UNDEFINED_TRANSACTION);
                resolve(transaction)

...
async function runMain(): Promise<boolean> {7
   ...
   if (!(await solana.optinAccountExists(solanaAccount, "xALGO"))) {
    ...
    //   const txn = await solana.bridgeTransaction(
    //     solanaAccount.addr,
    //     "USDC",
    //     "algorand",
    //     algorandAccount.addr,
   ...
   const bridge_transaction_algo_a =  await algorand.bridgeTransaction(algorandAccount.addr, "algo", "solana", solanaAccount.addr, "xalgo", 5);
   ...
   console.log("BRIDGE FINISHED STOP THE PROCESS")
   ...
   startingBalance = await algorand.getBalance(algorandAccount.addr);
   const bridge_txn_sol_a =   await solana.bridgeTransaction(solanaAccount.addr, "xalgo", "algorand", algorandAccount.addr, "algo", 5);
   if(!bridge_txn_sol_a){
   ...
   await Sleep(30000);
    ...
    const bridge_txn_sol_b = await solana.bridgeTransaction(solanaAccount.addr, "sol", "algorand", algorandAccount.addr, "xsol", 0.1);
    if(!bridge_txn_sol_b){
